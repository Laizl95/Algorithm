...
#define se second
using namespace std;
const int maxn=85,inf=1e9+5,mod=998244353;
...
inline BigNum maxx(const BigNum &a, const BigNum &b){
    if(a>b==1) return a;
    return b;
}
int a[maxn][maxn];
BigNum dp[maxn][maxn];
//dp[i][j]=max(dp[i-1][j]+(1<<k)*a[i-1],dp[i][j+1]+(1<<k)*a[j+1])
int main(){
    int n,m;
    scanf("%d %d",&n,&m);
    ...
    rep(i,1,n+1){
        ...
        rep(j,1,m){
            ...
            rep(k,1,m+1){
                int r=k+len-1;
                if(r>m) break;
                dp[k][r]=maxx(dp[k-1][r]+t*a[i][k-1],
                                dp[k][r+1]+t*a[i][r+1]);
            }
        }
        t=t*2;
        BigNum tmp=0;
        rep(j,1,m+1) tmp=maxx(dp[j][j]+t*a[i][j],tmp);
        //cout<<tmp<<endl;
        ans=ans+tmp;
        ms(dp,0);
    }
